...
app.get('/v/:id([a-z0-9_-]{10,20})/:format([a-z0-9]{3,4})?', (req, res) => {
	ytdl.getInfo(req.params.id, [], {maxBuffer: 100000 * 1024}, (err, info) => {
		if (err)
			return res.send(
				build(
					{
		...
		let format = info.formats.filter(format => {
			return (
				format.format_id ==
				config.formats[req.params.format || config.defaultFormat].id
			)
		})[0]
...
app.get(
	'/:channel([a-z0-9_-]{1,50})/:filter([a-z0-9_-]{1,50})?',
	(req, res) => {
		res.setHeader('content-type', 'application/rss+xml')
		...
		let format = config.formats[req.query.format]
			? req.query.format
			: config.defaultFormat
		let title = req.query.title ? req.query.title.substring(0, 50) : undefined

		new YouTube(channel, filter, format, title, host, path).then(
			data => {
